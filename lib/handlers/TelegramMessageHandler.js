...
class TelegramMessageHandler extends MessageHandler {
        ...
        client.on('message', async (ctx, next) => {
            if (this._enabled && ctx.message && ctx.chat) {
                ...
                if (ctx.message.is_topic_message && ctx.message.message_thread_id) {
                    context.to = `${ctx.chat.id}#${ctx.message.message_thread_id}`;
                }
    ...
    async _say(method, target, message, options = {}) {
        if (!this._enabled) {
            ...
            if (typeof target === 'string' && target.match(/#\d+$/)) {
                let tid;
                [, target, tid] = target.match(/^(.*)#(\d+)$/);
                options ??= {};
                options.is_topic_message = true;
                options.message_thread_id = Number.parseInt(tid);
            }
